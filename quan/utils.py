...
def quantizer(name,num_solution,T,default_cfg, this_cfg=None):
    ...
    q_ret.update_num_solution(num_solution,T)
    return q_ret


def find_modules_to_quantize(model, quan_scheduler,num_solution,T):
        ...
        if type(module) in QuanModuleMapping.keys():
            ...
            if name in quan_scheduler.excepts:
                ...
                replaced_modules[name] = QuanModuleMapping[type(module)](
                    ...
                    module,
                    quan_w_fn=quantizer(name+"weight",num_solution,T,quan_scheduler.weight,
                                        quan_scheduler.excepts[name].weight),
                    quan_a_fn=quantizer(name,num_solution,T,quan_scheduler.act,
                ...
                replaced_modules[name] = QuanModuleMapping[type(module)](
                    module,
                    quan_w_fn=quantizer(name,num_solution,T,quan_scheduler.weight),
                    quan_a_fn=quantizer(name,num_solution,T,quan_scheduler.act)
                )
